...
- **Target recovery paths:** WebView passkey/bank failures may require an external-browser handoff with deep-link return. These require deployment and real-device verification; the SDK alone does not implement or prove them. After an uncertain broadcast, reconcile before considering manual paste-to-send or another request.
